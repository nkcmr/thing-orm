...
module.exports = function init (connection) {
  ...
  function Thing () {
    ...
    _.forIn(schemaUtil.getRelated(schemas[this.kind]), function (opts, attr) {
  ...
  function make_new_thing (name, initialize) {
    ...
    schemas[name] = _schema._compile()

    _.extend(__, Super, _static, {
      extend: make_new_thing,
      make: function () {
        throw new ThingError(`'make' is only available on the base thing builder, use 'extend'`)
      }
    })
  ...
  function batchRelatedLoad (thingIds, attribute) {
    var self = this
    var kind = self.kind
    var related = schemaUtil.getRelated(schemas[kind])[attribute]
