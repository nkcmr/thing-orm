...
module.exports = function init (_config) {
  ...
  function queryComplete (mode, opts, rows) {
    ...
    if (rows.length === 0) {
      return Promise.resolve(FIND_MODE_FIRST ? null : [])
    }
  ...
  function find (mode, where, opts) {
    var self = this
    var stmt = knex(config.get(self[kind]).table)

    if (_.get(opts, 'select')) {
      stmt.select(opts.select)
    } else {
      stmt.select(_.map(schemas.get(self[kind])._attributes, 'name'))
    }
  ...
  function create_thing (opts) {
    ...
    return processHooks.call(self, 'before', 'save')
      .then(function () {
        return knex.insert(get_save_thing.call(self, SAVE_MODE_CREATE, opts), cfg.primaryKey)
          .into(cfg.table)
      })
      .then(function (pk) {
        if (_.isArray(pk)) {
        ...
        return save_complete.call(self, SAVE_MODE_CREATE)
      })
  }
  ...
  function update_thing (opts) {
    ...
    return processHooks.call(self, 'before', 'save')
      ...
      .then(function () {
        return save_complete.call(self, SAVE_MODE_UPDATE)
      })
  }
  ...
  function processHooks (order, event) {
    ...
    return Promise.all(_hooks.map(function (fn) {
